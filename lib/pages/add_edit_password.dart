...
class AddEditPassword extends StatefulWidget {
  AddEditPassword({Key key, this.pass}) : super(key: key);
  final Password pass;

  _AddEditPasswordState createState() => _AddEditPasswordState();
}
...
class _AddEditPasswordState extends State<AddEditPassword> {
  ...
  Future<Null> getPassUpdate() async {
    ...
    String hexCode = widget.pass.color.replaceAll('#', '');
    pickedColor = Color(int.parse('FF$hexCode', radix: 16));
    pickedIcon = iconNames.indexOf(widget.pass.icon);
  }
